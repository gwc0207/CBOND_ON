...
def test_catalogue_preserves_v5_and_adds_liquidity_composition_family() -> None:
    entries = batch_v6.factor_mining_catalog()

    assert len(entries) == 21
    assert len({entry.signal for entry in entries}) == 21
