...
from cbond_on.domain.factors.operators import research_factor_mining_cross_asset_event_sequence_topology_v1 as topology
...
def _increments(shape: int) -> list[float]:
    if shape == 0:
        return [1, 1, 2, 1, 3, 1, 2, 1, 10, 11, 12, 2, 1, 9, 8, 2, 1, 6, 3, 2, 8, 1, 2]
    if shape == 1:
        return [1, 5, 7, 8, 2, 1, 3, 1, 10, 11, 12, 1, 7, 2, 1, 8, 9, 1, 2, 6, 1, 7, 3]
    return [4, 1, 6, 2, 8, 1, 3, 7, 2, 9, 1, 5, 2, 8, 1, 6, 2, 9, 1, 4, 7, 2, 6]


def _panel(*, code: str, shape: int) -> pd.DataFrame:
    cumulative = 100.0
    rows: list[dict[str, object]] = []
    for seq, (clock, increment) in enumerate(zip(_times(), _increments(shape), strict=True)):
...
def test_topology_catalogue_registration_and_context_contract() -> None:
    entries = topology.factor_mining_catalog()

    assert len(entries) == 6
    assert len({entry.signal for entry in entries}) == 6
    assert Counter(entry.family for entry in entries) == {
        "cross_asset_relative_event_order": 3,
        "cross_asset_burst_cluster_topology": 3,
    }
    assert FactorRegistry.get(topology.KERNEL_NAME) is topology.FactorMiningCrossAssetEventSequenceTopologyV1
    assert set(topology.FORMULAS) == {entry.signal for entry in entries}
    assert topology.FactorMiningCrossAssetEventSequenceTopologyV1.requires_stock_panel is True
    assert topology.FactorMiningCrossAssetEventSequenceTopologyV1.requires_bond_stock_map is False


def test_topology_kernel_builds_six_finite_nonreturn_signals_from_tminus1_mapping() -> None:
    frame = _build()
    ...
    assert 0.0 <= frame.loc[(DT, BOND), "xca_relative_activity_leader_entropy"] <= 1.0
    assert -1.0 <= frame.loc[(DT, BOND), "xca_relative_activity_leader_lag2_agreement"] <= 1.0


def test_topology_uses_certified_tminus1_mapping_and_ignores_context_score_and_future_rows() -> None:
    ...
    assert baseline.loc[(DT, BOND), "xca_event_bin_entropy_gap"] != pytest.approx(
        tminus1_other_stock.loc[(DT, BOND), "xca_event_bin_entropy_gap"]
    )


def test_topology_fails_closed_for_stale_mapping_without_inf() -> None:
    prior = DT.normalize() - pd.offsets.BDay(1)
...
def test_topology_ignores_nonphysical_preopen_and_after_cutoff_rows() -> None:
    baseline = _build()
...
def test_topology_counter_reset_becomes_nan_not_infinite() -> None:
...
def test_topology_missing_field_and_no_events_fail_closed() -> None:
    missing = _panel(code=BOND, shape=0).drop(columns=["num_trades"])
    missing.attrs["__build_day__"] = DT.date().isoformat()
    missing_result = _build(bond_panel=missing)

    no_events = _panel(code=BOND, shape=0).reset_index()
    ...
    no_events_result = _build(bond_panel=no_events_panel)

    assert missing_result.isna().all(axis=None)
    assert no_events_result.isna().all(axis=None)


def test_topology_rejects_unknown_signal() -> None:
    with pytest.raises(KeyError, match="unknown signal"):
        topology.FactorMiningCrossAssetEventSequenceTopologyV1().compute(
