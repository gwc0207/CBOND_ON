...
class SnapshotLoader:
    """
    使用 snapshot 数据生成日内换仓窗口的面板数据：
    index = (dt, code)
    columns 包含 signal_* �?window_* 字段，以�?twap�?
    """
    ...
    def _build_day_panel(self, path: Path) -> Optional[pd.DataFrame]:
        ...
        for start_t, end_t in self.schedule.windows:
            ...
            # 使用 left join：即使窗口内没有成交（无 TWAP），也保�?signal_* 用于估值，
            # 避免在午�?低流动性窗口出现持仓被“估值为 0”的断崖式跳变�?
            merged = signal.join(twap, how="left").join(post, how="left")
            twap_col = twap.columns[0]
            if self.config.drop_no_trade:
                # 默认“剔除无成交”解释为：无成交时不允许用于选券/成交（置 twap �?NA），
                # 但保�?signal_*，用于持仓估值与因果性检查�?
            ...
            # merged �?index �?code；将 dt 作为另一�?index，得�?(dt, code)
            merged = merged.copy()
            # dt 使用节点时间（窗口开始时刻）
    ...
    def _window_twap(
        ...
        if self.config.twap_method != "time":
            raise ValueError(f"暂不支持 twap_method={self.config.twap_method}")
