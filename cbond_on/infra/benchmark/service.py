...
def compute_strict_cycle_detail_for_holdings(
    *,
    ...
    detail["return_net"] = (1.0 + buy_net) * (1.0 + sell_net) - 1.0
    detail["return_gross"] = (1.0 + buy_gross) * (1.0 + sell_gross) - 1.0
...
def compute_benchmark_detail_for_day(
    *,
    ...
    return compute_benchmark_cycle_detail_for_day(
        raw_data_root=raw_data_root,
        buy_day=trade_day,
        sell_day=next_day,
        buy_bps=buy_bps,
    ...
    *,
    ...
    try:
        ...
        if not skip_failed_days:
            raise
        print("[benchmark] next-day map failed; returning empty", flush=True)
        return pd.DataFrame()
    ...
    for idx, day in enumerate(days, start=1):
        ...
        try:
            ...
            if detail.empty:
                raise RuntimeError(f"benchmark strict cycle detail empty: buy_day={day:%Y-%m-%d}")
        except Exception:
            if not skip_failed_days:
                raise
            skipped += 1
        ...
        buy_gross_return = float(pd.to_numeric(detail.get("weighted_buy_leg_ret_gross"), errors="coerce").sum())
        sell_net_return = float(pd.to_numeric(detail["weighted_sell_leg_ret_net"], errors="coerce").sum())
        sell_gross_return = float(pd.to_numeric(detail.get("weighted_sell_leg_ret_gross"), errors="coerce").sum())
        benchmark_return = float(pd.to_numeric(detail["weighted_return"], errors="coerce").sum())
        nav *= 1.0 + benchmark_return

        buy_weight = pd.to_numeric(detail["weight"], errors="coerce").fillna(0.0)
        fallback_mask = detail["sell_missing_fallback"].astype(bool)
        rows.append(
            {
                "trade_date": day,
                "trade_day": day,
                "sell_trade_day": next_day,
                "buy_trade_day": day,
                "prev_buy_trade_day": pd.NaT,
                "next_day": next_day,
                ...
                "strict_sell_fee_weighted": float(buy_weight.sum() * bps_to_rate(sell_bps)),
                ...
                "fallback_sell_weight": float(buy_weight.loc[fallback_mask].sum()),
...
def compute_benchmark_breakdown_for_day(
    *,
    ...
    return BenchmarkReturnBreakdown(
        ...
        sell_count=int(detail["code"].nunique()),
        fallback_sell_codes=int(fallback_mask.sum()),
        fallback_sell_weight=float(pd.to_numeric(detail.loc[fallback_mask, "weight"], errors="coerce").sum()),
    )
